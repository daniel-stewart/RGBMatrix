...
try:
    ...
    print("fontdir_prefix is kept in secrets.py, please add them.")
...
class MatrixSprite(MatrixBase):
    ...
    def initialize(self, width, height, double_buffer):
        ...
        self.font.LoadFont(self.fontPathPrefix + "6x13.bdf")
        if self.level == 3:
    ...
    def run(self, double_buffer):
        ...
        double_buffer.Clear()
        double_buffer.SetImage(self.background, 0)
        if self.text != '' or self.text2 != '':
            graphics.DrawText(double_buffer, self.font, 3, 48, graphics.Color(0, 255, 255), self.text)
            graphics.DrawText(double_buffer, self.font, 3, 62, graphics.Color(0, 255, 255), self.text2)
        if self.level == 2:
            graphics.DrawText(double_buffer, self.font, 3, 53, graphics.Color(0,255,0), "Goodnight!")
        return 1
