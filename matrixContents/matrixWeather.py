...
class MatrixWeather(MatrixBase):
    def __init__(self, number):
        ...
        self.OWMError = False
        self.yOffset = 0 if self.number == 0 else 32
    ...
    def createDay(self, day, doubleBuffer, color):
        ...
        graphics.DrawText(doubleBuffer, font, 10, 20 + self.yOffset, color, day)
        return

    def initialize(self, width, height, doubleBuffer):
        self.iconPathPrefix = secrets['icondir_prefix'] # Original: '/home/pi/icons/weatherIcons/png/24x24/'
        self.weatherTable = {
        ...
        if self.bSecretFileExists:
            ...
            if self.OWMError:
                ...
                graphics.DrawText(doubleBuffer, f, 8,22 + self.yOffset, white, "Hello")
                graphics.DrawLine(doubleBuffer, 0, 0 + self.yOffset, 0, 31 + self.yOffset, white)
                graphics.DrawLine(doubleBuffer, 1, 0 + self.yOffset, 1, 31 + self.yOffset, white)
                graphics.DrawLine(doubleBuffer, 63, 0 + self.yOffset, 63, 31 + self.yOffset, white)
                graphics.DrawLine(doubleBuffer, 62, 0 + self.yOffset, 62, 31 + self.yOffset, white)
                graphics.DrawLine(doubleBuffer, 2, 0 + self.yOffset, 61, 0 + self.yOffset, white)
                graphics.DrawLine(doubleBuffer, 2, 1 + self.yOffset, 61, 1 + self.yOffset, white)
                graphics.DrawLine(doubleBuffer, 2, 30 + self.yOffset, 61, 30 + self.yOffset, white)
                graphics.DrawLine(doubleBuffer, 2, 31 + self.yOffset, 61, 31 + self.yOffset, white)
                return
        ...
        graphics.DrawText(doubleBuffer, self.font, 35, 18 + self.yOffset, graphics.Color(*self.colorRamp(self.currentTemp)), str(self.currentTemp)+"\u00B0")
        graphics.DrawText(doubleBuffer, self.font2, 0, 30 + self.yOffset, graphics.Color(*WHITE), self.weatherTable[self.wid][1])
        return
    ...
    def restart(self, doubleBuffer):
        ...
        if self.now < datetime.now():
            try:
                ...
                graphics.DrawText(doubleBuffer, f, 8,22 + self.yOffset, green, "Hello")
                graphics.DrawLine(doubleBuffer, 0, 0 + self.yOffset, 0, 31 + self.yOffset, white)
                graphics.DrawLine(doubleBuffer, 1, 0 + self.yOffset, 1, 31 + self.yOffset, white)
                graphics.DrawLine(doubleBuffer, 63, 0 + self.yOffset, 63, 31 + self.yOffset, white)
                graphics.DrawLine(doubleBuffer, 62, 0 + self.yOffset, 62, 31 + self.yOffset, white)
    ...
    def run(self, doubleBuffer):
        ...
        if self.now < datetime.now():
            ...
            doubleBuffer.Clear()
            doubleBuffer.SetImage(image,0)
            graphics.DrawText(doubleBuffer, self.font, 35, 18 + self.yOffset, graphics.Color(*self.colorRamp(currentTemp)), str(currentTemp)+"\u00B0")
            graphics.DrawText(doubleBuffer, self.font2, 0, 30 + self.yOffset, graphics.Color(*WHITE), self.weatherTable[wid][1])
            return 1
        return 0
